...
class InspectionSheet(models.Model):
    ...
    final_m1_m2 = fields.Float('Final Result', digits=dp.get_precision('Quality Reports'),
                               compute='compute_final_m1_m2')
    final_m1_m2_label = fields.Float('Final Result', digits=dp.get_precision('Quality Reports'),
                                     compute='compute_final_m1_m2')
    ...
    final_result_label = fields.Float('Final Result', digits=dp.get_precision('Quality Reports'),
        ...
        for sheet in self:
            if sheet.state == 'approved':
                if not sheet.first_approved_by or not sheet.second_approved_by:
                    for message in sheet.sudo().message_ids:
                        for track in message.sudo().tracking_value_ids:
                            if track.old_value_char == 'Waiting First Approval' and track.new_value_char == 'Waiting Second Approval':
                                ...
                                sheet.sudo().write({'first_approved_by': fuser.id})
                                ...
                                secuser = self.env['res.users'].sudo().search(
                                    [('partner_id', '=', track.mail_message_id.author_id.id)])
                                sheet.sudo().write({'second_approved_by': secuser.id})
